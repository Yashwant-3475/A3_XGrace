...
// POST /api/ai-interview/save-session — persist all 5 answers at the end of an AI interview
const saveSession = async (req, res) => {
    try {
        ...
        const percentage = Math.round(averageScore * 10);
        const skillLevel = computeSkillLevel(averageScore);

        const session = await AiInterviewSession.create({ userId: req.user.id, role, answers, totalQuestions, averageScore, percentage, skillLevel });

        res.status(201).json({
            sessionId: session._id, role: session.role,
            averageScore: session.averageScore, percentage: session.percentage,
            skillLevel: session.skillLevel, totalQuestions: session.totalQuestions,
...
// GET /api/ai-interview/recent — last 5 sessions for the dashboard (answers excluded)
const getRecentSessions = async (req, res) => {
    try {
        const sessions = await AiInterviewSession.find({ userId: req.user.id })
            .sort({ createdAt: -1 }).limit(5).select('-answers').lean();
...
// GET /api/ai-interview/history — paginated + filterable history for the authenticated user
const getHistory = async (req, res) => {
    try {
        ...
        if (req.query.minScore) filter.averageScore = { $gte: parseFloat(req.query.minScore) };
        ...
        const sessions   = await AiInterviewSession.find(filter).sort({ createdAt: -1 }).skip(skip).limit(limit).lean();

        res.status(200).json({ page, totalPages, totalItems, sessions });
    } catch (error) {
...
// GET /api/admin/ai-stats — platform-wide aggregate analytics (admin only)
const getAdminStats = async (req, res) => {
    try {
        const [totalSessions, avgScoreResult, roleDistRaw, skillDistRaw, topPerformerRaw, dailyRaw] = await Promise.all([
            ...
            AiInterviewSession.aggregate([{ $group: { _id: null, avg: { $avg: '$averageScore' } } }]),
            AiInterviewSession.aggregate([{ $group: { _id: '$role', count: { $sum: 1 } } }, { $sort: { count: -1 } }]),
            AiInterviewSession.aggregate([{ $group: { _id: '$skillLevel', count: { $sum: 1 } } }]),
        ...
        res.json({
            totalSessions, platformAvgScore, mostPopularRole, topPerformer,
...
// GET /api/admin/ai-sessions — paginated list of all users' sessions (admin only)
const getAllSessions = async (req, res) => {
    try {
        ...
        // Join with users collection to support name/email search
        let pipeline = [
            { $match: filter },
        ...
        const [countResult, sessions] = await Promise.all([
            ...
            AiInterviewSession.aggregate([
                ...
                { $project: { role: 1, averageScore: 1, percentage: 1, skillLevel: 1, totalQuestions: 1, createdAt: 1, answers: 1, 'user.name': 1, 'user.email': 1, 'user._id': 1 } },
            ]),
        ]);
...
// DELETE /api/admin/ai-sessions/:id — remove a session by ID (admin only)
